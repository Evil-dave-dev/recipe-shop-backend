...
const checkBody = (data)=>{
    const {name, instructions, ingredients, dishType, preparationTime, difficulty, regime, tags} = data
    if(!name || !name.length > 1 ) return{result: false, error: "'name' must be a non-null string"}
    ...
    if(!EnumsRecipes.tags.some(tags)) return{result: false, error: `invalidd 'tags', please use: ${EnumsRecipes.tags.join(". ")}`}
    if(!EnumsRecipes.regime.some(regime)) return{result: false, error: `invalidd 'tags', please use: ${EnumsRecipes.regime.join(". ")}`}
